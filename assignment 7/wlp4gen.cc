...
std::string lvalue_id(TreeNode* node) {
  if (node->children.size() == 0) {
    return node->data[1];
  }
  else {
    std::string ret = "";
    if (node->data[0] == "lvalue") {
      if (node->children.size() == 1) {
        if (node->data[1] == "ID") {
          ret = lvalue_id(node->children[0]);
        }
      }
      else if (node->children.size() == 3) {
        if (node->data[1] == "LPAREN" && node->data[2] == "lvalue" && node->data[3] == "RPAREN") {
          ret = lvalue_id(node->children[1]);
        }
      }
    }
    return ret;
...
void code(TreeNode* node, std::unordered_map<std::string, int> offset_map) {
  ...
  if (node->children.size() == 0) {
    ...
    if (node->data[0] == "expr") {
      ...
      if (node->children.size() == 1) {
        if (node->data[1] == "NUM" || node->data[1] == "ID") {
          code(node->children[0], offset_map);
        }
      }
      else if (node->children.size() == 3) {
      ...
      if (node->children.size() == 4) {
        if (node->data[1] == "lvalue" && node->data[2] == "BECOMES" && node->data[3] == "expr" && node->data[4] == "SEMI") {
          code(node->children[2], offset_map);
          std::cout << "sw $3, " << offset_map[lvalue_id(node->children[0])] << "($29)" << std::endl;
        }
      }
...
int main() {
  ...
  std::unordered_map<std::string, std::unordered_map<std::string, int>> procedure_map;      // can replace with procedure later
  ...
  code(curr_node->children[9], wain_table);

  code(curr_node->children[11], wain_table);
